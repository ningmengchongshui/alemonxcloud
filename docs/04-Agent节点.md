...
make agent-build VERSION=v1.0.17
...
| 容器生命周期 | `POST /container/create`、`/:name/start`、`/:name/stop`、`/:name/restart`、`/:name/reinstall`、`/:name/destroy`、`DELETE /:name?purge=true` | 创建、启停、按当前控制面配置重建重启（包括旧目录布局迁移）、清空数据后重装、销毁运行资源，或在保留期后清理数据 |
