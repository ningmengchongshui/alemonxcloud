...
export function Shell({ children, user, area, page, onPageChange, superPage, onSuperPageChange, onGoToMe, onGoToSuper, onLogout }: PropsWithChildren<ShellProps>) {
  ...
  const superNav: Array<[SuperPage,string,string]> = [['overview','◇','总览'],['orders','□','订单记录'],['tasks','↻','任务队列'],['catalog','▤','商品目录'],['nodes','◌','节点管理'],['users','♙','用户与钱包'],['audit','◷','安全审计']]
  const nav = area === 'me' ? userNav : superNav
  const activeLabel = nav.find(([key]) => key === (area === 'me' ? page : superPage))?.[2]
  return <div className={`shell shell-${area}`}><aside className="side"><div className="brand"><BrandLogo /></div><nav aria-label={area === 'me' ? '用户控制台导航' : '超级管理台导航'}>{nav.map(([key,icon,label])=><button key={key} title={label} aria-label={label} className={classNames({active:(area==='me'?page:superPage)===key})} aria-current={(area==='me'?page:superPage)===key?'page':undefined} onClick={()=>area==='me'?onPageChange?.(key as Page):onSuperPageChange?.(key as SuperPage)}><span className="icon" aria-hidden="true">{icon}</span><span className="nav-label">{label}</span></button>)}</nav><div className="side-bottom">{area==='me'&&onGoToSuper&&<button className="workspace-switch" title="进入超级管理台" aria-label="进入超级管理台" onClick={onGoToSuper}><span className="icon" aria-hidden="true">◇</span><span className="nav-label">进入超级管理台</span></button>}{area==='super'&&<button className="workspace-switch" title="返回用户控制台" aria-label="返回用户控制台" onClick={onGoToMe}><span className="icon" aria-hidden="true">←</span><span className="nav-label">返回用户控制台</span></button>}<div className="profile-menu" ref={profileRef}>{profileOpen&&<section className="profile-card" id="profile-card" aria-label="账户信息"><div className="profile-card-user"><i>{displayName.slice(0,2).toUpperCase()}</i><div><b>{displayName}</b><span>{area==='super'?'超级管理员':'个人账户'}</span></div></div><p>{area==='super'?'你正在管理平台运营与交付。':'在这里管理你的服务和订阅。'}</p><button className="profile-logout" onClick={onLogout}><span>↗</span>退出登录</button></section>}<button className="profile" onClick={()=>setProfileOpen(value=>!value)} aria-expanded={profileOpen} aria-controls="profile-card"><i>{displayName.slice(0,2).toUpperCase()}</i><span><b>{displayName}</b><small>{area==='super'?'超级管理员':'个人账户'}</small></span><span className="profile-chevron" aria-hidden="true">{profileOpen?'⌃':'⌄'}</span></button></div></div></aside><main className="main"><header className="console-topbar"><span>{area === 'super' ? '超级管理台' : '用户控制台'}</span><i>/</i><b>{activeLabel}</b><small>{area === 'super' ? '生产环境' : '个人工作区'}</small></header>{unread>0&&<div className="notice-bar" role="status"><span>你有 {unread} 条未读站内通知</span><button onClick={()=>void readAll()}>全部标为已读</button></div>}{children}</main></div>
}
