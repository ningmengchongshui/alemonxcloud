      ...
      <div className="space-y-3 p-3">
        ...
        <section aria-label="选择优惠券">
          <div className="mb-1.5 flex items-baseline justify-between gap-3">
            <h4 className="m-0 text-[11px] font-bold text-slate-700 dark:text-slate-100">选择优惠券</h4>
            <span className="text-[10px] text-slate-400">展示券包全部可用券</span>
          </div>
          {coupons.isLoading ? <p className="m-0 text-[11px] text-slate-500">正在载入券包…</p> : availableCoupons.length === 0 ? (
            <p className="m-0 text-[11px] leading-5 text-slate-500 dark:text-slate-300">券包中没有待使用优惠券；本单将按原价结算。</p>
          ) : (
            <div className="grid gap-1.5">
              {availableCoupons.map(coupon => {
                ...
                return <button key={coupon.id} type="button" disabled={!usable} aria-pressed={selected} onClick={() => usable && onSelect(coupon.id, false)} className={classNames('flex items-center justify-between gap-3 rounded-md border px-3 py-2.5 text-left transition-colors focus-visible:outline-2 focus-visible:outline-blue-500', selected ? 'border-blue-500 bg-blue-50 dark:bg-blue-950' : usable ? 'border-slate-200 hover:border-blue-300 dark:border-slate-700' : 'cursor-not-allowed border-slate-100 bg-slate-50 opacity-60 dark:border-slate-800 dark:bg-slate-900')}>
                  <span className="min-w-0"><b className="block truncate text-xs text-slate-800 dark:text-white">{coupon.batch?.name ?? '历史优惠券'}</b><small className="mt-0.5 block truncate text-[10px] text-slate-500 dark:text-slate-300">{couponRule(coupon)} · {usable ? '适用于当前订单' : '不适用于当前套餐或周期'}</small></span>
                  {candidate && <span className="shrink-0 text-right text-[10px]"><b className="block text-emerald-700 dark:text-emerald-200">减 <XCoinAmount value={(candidate.discountAmountFen / 100).toFixed(2)} /></b><small className="mt-0.5 block text-slate-400">实付 {(candidate.payableAmountFen / 100).toFixed(2)}</small></span>}
                </button>
              })}
            </div>
          )}
