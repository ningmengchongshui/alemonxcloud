...
export function InstanceTerminalPage({
  ...
  useEffect(() => {
    ...
    const terminal = new Terminal({
      ...
      theme: { background: '#020617', foreground: '#e2e8f0', cursor: '#93c5fd' }
    })
    ...
    socket.onerror = () =>
      setError('终端连接失败，请确认实例和节点均处于运行状态。')
  ...
  if (!instance)
    return (
      <EmptyState
        title="实例不存在"
        description={`未找到实例 ${instanceID}。`}
        action={<Button onClick={onBack}>返回实例</Button>}
      />
    )
  return (
    <section className="flex h-dvh flex-col overflow-hidden bg-slate-50 p-2 dark:bg-slate-950">
      <div className="flex shrink-0 items-center justify-between gap-3 pb-2">
        ...
        />
        {mode === 'files' && (
          <span className="hidden min-w-0 flex-1 truncate text-right text-[11px] text-slate-500 sm:block">
            /app/workspace{path ? `/${path}` : ''}
          </span>
        )}
        <Button tone="ghost" className="min-h-8 px-2.5" onClick={onBack}>
          返回
        </Button>
      </div>
      <main className="min-h-0 flex-1">
        {!ready && mode === 'terminal' ? (
          <Alert tone="info">
            终端将在实例运行后可用，请等待部署或启动完成。
          </Alert>
        ) : mode === 'terminal' ? (
          <div className="relative h-full">
            <div
              ref={host}
              className="h-full overflow-hidden rounded-xl border border-slate-800 bg-slate-950 p-3 shadow-sm"
              aria-label="容器终端"
            />
            {error && (
          ...
          <section className="grid h-full min-h-0 grid-cols-[17rem_1fr] overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 max-[850px]:h-auto max-[850px]:grid-cols-1 max-[850px]:overflow-visible">
            <aside className="flex min-h-0 flex-col border-r border-slate-200 p-3 dark:border-slate-700 max-[850px]:border-r-0 max-[850px]:border-b">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <label className="cursor-pointer text-[11px] font-bold text-blue-700">
                  <input
                    className="sr-only"
                    type="file"
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                {filesLoading ? (
                  <LoadingState>正在读取文件…</LoadingState>
                ) : (
                  <div className="space-y-1">
                    {listing?.entries.map(entry => (
                      <button
                        ...
                        className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left text-xs hover:bg-blue-50 dark:hover:bg-slate-700"
                      >
                ...
                <>
                  <div className="mb-2 flex shrink-0 items-center justify-between gap-3">
                    ...
                    <Button
                      loading={saving}
                      onClick={() =>
                        void saveFile({ id: instanceID, ...editor })
                          .unwrap()
                          .then(() => void refetch())
                      }
                    >
                  ...
                  <textarea
                    ...
                    spellCheck={false}
                    className="min-h-0 flex-1 resize-none rounded-lg border border-slate-200 bg-slate-950 p-3 font-mono text-xs leading-6 text-slate-100 outline-none focus:border-blue-500 dark:border-slate-600 max-[850px]:min-h-96"
                  />
                </>
              ) : (
                <EmptyState
