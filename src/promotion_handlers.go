...
func quotePurchaseHandler(c *gin.Context) {
	...
	q, _, _, err := quoteForModern(c, user.ID, "purchase", b.PlanID, b.ImageID, b.Months, monthly*b.Months, b.SelectionID, b.PayFullPrice, nil)
	if err != nil {
		businessError(c, err)
		return
	}
...
func claimPromotionHandler(c *gin.Context) {
	c.JSON(http.StatusGone, gin.H{"message": "旧活动领券入口已停用，请使用优惠中心"})
	return
	/*
		user := c.MustGet("user").(oidcUser)
...
func quoteRenewHandler(c *gin.Context) {
	...
	q, _, _, err := quoteForModern(c, user.ID, "renewal", planID, imageID, b.Months, monthly*b.Months, b.SelectionID, b.PayFullPrice, nil)
	if err != nil {
		businessError(c, err)
		return
	}
...
func adminCreateCoupons(c *gin.Context) {
	c.JSON(http.StatusGone, gin.H{"message": "旧券码接口已停用，请使用代金券批次"})
	return
	/*
		var b struct {
